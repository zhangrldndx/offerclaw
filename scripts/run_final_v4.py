#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run the three pre-registered Final v4 arms and report the full metric panel.

Final v4 exists to convert two already-validated candidates into defaults:
A = today's production default (B0 + v4 prompt); B = A with the v5 judge
prompt; C = the quality package (HyDE channels + 3-vote consensus gate + v5).
Original v3 header follows for the shared mechanics.
...
# Judge is default-ON since 2026-08-30; arms only override what differs.
ARMS = {
    "A": ("compact32_pool28", {}, True, False),
    "B": ("compact32_pool28", {"RAG_ANSWERABILITY_PROMPT": "v5"}, True, False),
    "C": ("compact32_pool28_hydechan_bm25",
          {"RAG_ANSWERABILITY_PROMPT": "v5", "RAG_ANSWERABILITY_GATE": "1",
           "RAG_ANSWERABILITY_GATE_VOTES": "3"}, True, True),
}
JUDGE_ENV = ("RAG_ANSWERABILITY", "RAG_ANSWERABILITY_EARLY_EXIT",
             "RAG_ANSWERABILITY_DEPTH", "RAG_ANSWERABILITY_PROMPT")
...
def _check_sentinels(payload: dict, expect_judge: bool, name: str) -> None:
    ...
    if expect_judge and applied != len(rows):
        raise SystemExit(f"[final-v4] 哨兵失败 {name}: 判据只在 {applied}/{len(rows)} 行生效")
    if not expect_judge and applied:
        raise SystemExit(f"[final-v4] 哨兵失败 {name}: 该臂不该开判据,却有 {applied} 行生效")
    if degraded:
        raise SystemExit(f"[final-v4] 哨兵失败 {name}: {degraded} 行裁判掉线")
...
def main() -> None:
    ...
    parser.add_argument("--outdir", default="~/.offerclaw/private_eval/final_v4/runs")
    parser.add_argument("--summary", default="docs/rag_eval/final_v4/runs/SUMMARY.json")
    args = parser.parse_args()

    dataset = ROOT / args.dataset
    data = json.loads(dataset.read_text(encoding="utf-8"))
    if data.get("status") != "approved":
        raise SystemExit(f"[final-v4] 数据集 status={data.get('status')!r},需人工确认后 approved")
    expected = {i["query_id"]: i.get("expected_behavior")
                for i in data["items"] if i["case_kind"] == "negative"}

    verify = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "freeze_final_v4_config.py"), "--verify"],
        cwd=ROOT, capture_output=True, text=True)
    print(verify.stdout.strip() or verify.stderr.strip())
    if verify.returncode != 0:
        raise SystemExit("[final-v4] 冻结件与工作树不一致,拒绝运行")
        ...
        for repeat in range(1, args.repeats + 1):
            ...
            if out.exists():
                ...
                try:
                    ...
                    print(f"[final-v4] {arm} run {repeat} 已存在,跳过", flush=True)
            ...
            print(f"[final-v4] {arm} run {repeat}/{args.repeats} (arm={profile_arm})", flush=True)
            proc = subprocess.run(
            ...
            if proc.returncode != 0:
                raise SystemExit(f"[final-v4] {arm} run {repeat} 失败")
            payload = json.loads(out.read_text(encoding="utf-8"))["runs"][0]
            _check_sentinels(payload, expect_judge, out.name)
            results.setdefault(arm, []).append(_row(repeat, payload, expected))
    ...
    report = {"schema_version": "final-v4-report-v1", "repeats": args.repeats,
    ...
    summary_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n",
                            encoding="utf-8")
    print(f"\n[final-v4] 汇总 -> {summary_path.relative_to(ROOT)}")
